...
<pre><code>string preprocess_code(const int argc, const char** argv, const string& raw_code) {
    ...
    tcc_in_memory_preprocess(// the just created tcc state object - thanks to this, tccpp is multi-
							 // threading capable, i.e. you can create a TCCState object for each
							 // thread and then call tcc_in_memory_preprocess from each thread with
							 // their resp. state object
                             state,
                             // code input string + length
                             code_input, raw_code.length(),
                             // whether to print/add the include stack or not
                             true,
                             // the filename that should be used for this in-memory code (raw_code)
